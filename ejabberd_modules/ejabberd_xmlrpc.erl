...
handler(_State, {call, digime_send_announcement, [{struct, AttrL}]}) ->
	[{from, From}, {users, Users}, {id, Id}, {title, Title}, {body, Body}]=AttrL,
	{array, UsersList} = Users,
	lists:foreach( fun(User) -> mod_digime:send_message_announcement(From, User, Id, Title, Body) end, UsersList),
	{false, {response, [{struct, [{res, 0}]}]}};

handler(_State, {call, digime_send_notification, [{struct, AttrL}]}) ->
	[{from, From}, {users, Users}, {payload, Payload}]=AttrL,
	{array, UsersList} = Users,
	lists:foreach( fun(User) -> mod_digime:send_message_notification(From, User, Payload) end, UsersList),
	{false, {response, [{struct, [{res, 0}]}]}};
	
handler(_State, {call, digime_send_livepoll, [{struct, AttrL}]}) ->
	[{from, From}, {users, Users}, {payload, Payload}]=AttrL,
	{array, UsersList} = Users,
	lists:foreach( fun(User) -> mod_digime:send_message_livepoll(From, User, Payload) end, UsersList),
	{false, {response, [{struct, [{res, 0}]}]}};

handler(_State, {call, digime_send_liveslide, [{struct, AttrL}]}) ->
	[{from, From}, {users, Users}, {payload, Payload}]=AttrL,
	{array, UsersList} = Users,
	lists:foreach( fun(User) -> mod_digime:send_message_liveslide(From, User, Payload) end, UsersList),
	{false, {response, [{struct, [{res, 0}]}]}};
